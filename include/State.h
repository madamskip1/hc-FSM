...
namespace FSM
{
	...
	template <typename StateType, typename EventType>
	struct has_onExit<StateType, EventType, std::void_t<decltype(std::declval<StateType>().onEntry(std::declval<EventType>()))>> : std::true_type {};
	...
	template <typename StateType>
	struct has_onExitNoEventArg<StateType, std::void_t<decltype(std::declval<StateType>().onEntry())>> : std::true_type {};
